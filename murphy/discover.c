...
void pa_discover_register_sink_input(struct userdata *u, pa_sink_input *sinp)
{
    ...
    data.type      = type;
    data.zone      = pa_utils_get_zone(sinp->proplist, client);
    data.visible   = true;
    data.available = true;
                                     ...
                                     pa_sink_input_new_data *data)
{
    ...
    if (!data->sink) {
        ...
        fake.zone      = pa_utils_get_zone(data->proplist, client);
        fake.visible   = true;
        fake.available = true;
...
void pa_discover_add_sink_input(struct userdata *u, pa_sink_input *sinp)
{
    ...
    if (!strncmp(media, combine_pattern, sizeof(combine_pattern)-1)) {
        ...
        data.channels  = sinp->channel_map.channels;
        data.type      = type;
        data.zone      = pa_utils_get_zone(pl, client);
        data.visible   = true;
        data.available = true;
        data.amname    = get_stream_amname(type, name, pl);
        data.amdescr   = pa_proplist_gets(pl, PA_PROP_MEDIA_NAME);
                                        ...
                                        pa_source_output *sout)
{
    ...
    data.type      = type;
    data.zone      = pa_utils_get_zone(sout->proplist, client);
    data.visible   = true;
    data.available = true;
    data.amname    = name;
    data.amdescr   = pa_proplist_gets(pl, PA_PROP_MEDIA_NAME);
